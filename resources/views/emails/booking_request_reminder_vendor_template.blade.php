         ...
         <div style="background: #fff;padding: 15px;">
            <div style="padding:30px;border:1px solid #fff;background:#eafdfc;">
               ...
               <div style=" display:block; overflow:hidden">
                  <p class="parag">New Booking Request has been Arrived</p>
                  <p class="parag">Please approve and issue the invoice before</p>
                  @if(!empty($last_date))
                     <p class="parag">Date : {{$last_date ?? ''}}  &  Time : {{$last_time ?? ''}} & Remaining Time : {{$time_remaining ?? ''}}</p>
                  @else
                     <p class="parag">Booking will be cancelled at {{$last_date ?? ''}} {{$last_time ?? ''}} or on check in time</p>
                  @endif
