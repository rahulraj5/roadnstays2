...
<script>
  $(".space_up_submit").click(function(e) {
    var form = $("#updatespaceVendor_form");
    form.validate({
      rules: {
        category_id: {
          required: true,
    ...
    if (form.valid() === true) {
      ...
      $(form).ajaxSubmit({
        type: 'POST',
        url: site_url + '/servicepro/updateSpace',
        data: formData,
  ...
  function deleteConfirmation(Id) {
    toastDelete.fire({}).then(function(e) {
      if (e.value === true) {
        ...
        $.ajax({
          type: 'POST',
          url: "{{url('/servicepro/deleteSpaceSingleImage')}}",
          data: {
            'id': Id,
            _token: CSRF_TOKEN
          },
    ...
    <div class="col-md-12">
      <div class="card card-default">
        ...
        <div class="card-body">
          ...
          <form method="POST" id="updatespaceVendor_form" enctype="multipart/form-data">
