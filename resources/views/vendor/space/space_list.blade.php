  ...
  function deleteConfirmation(id) {
    toastDelete.fire({}).then(function(e) {
      if (e.value === true) {
        ...
        $.ajax({
          type: 'POST',
          url: "{{url('/servicepro/deleteSpace')}}",
          data: {
            id: id,
            _token: CSRF_TOKEN
          },
...
<script>
  $('.toggle-class').on('change', function() {
    ...
    $.ajax({
      type: "GET",
      dataType: "json",
      url: "<?php echo url('/servicepro/changeSpaceStatus'); ?>",
      data: {
        'status': status,
        'space_id': space_id,
      },
  ...
  function copyConfirmation(id) {
    toastCopy.fire({}).then(function(e) {
      if (e.value === true) {
        ...
        $.ajax({
          type: 'POST',
          url: "{{url('/servicepro/addCopySpace')}}",
          data: {
            space_id: id,
            _token: CSRF_TOKEN
          },
          ...
          success: function(results) {
            ...
            if (results.status == 'success') {
              ...
              setTimeout(function() {
                window.location.href = site_url + "/servicepro/edit-space/" + results.new_space_id
              }, 2000);
            } else {
  ...
  <div class="col-12">
    ...
    <!-- <div class="row">
      ...
      <div class="col-md-1">
        ...
        <a href="{{ url('/servicepro/add-space/') }}" class="btn btn-primary btn-dark btn_add htl-add">Add</a>
        ...
        <table id="example1" class="table table-bordered table-striped rm-listing">
          ...
          <tbody>
            ...
            <tr id="row{{ $arr->space_id }}">
              ...
              <td>
                <a href="javascript:void(0)" onclick="copyConfirmation('<?php echo $arr->space_id; ?>');" class="btn btn-info"><i class="bx bxs-copy" alt="copy" title="copy"></i></a>
              </td>
              ...
              <td class="text-center py-0 align-middle">
                ...
                <div class="btn-group btn-group-sm">
                  ...
                  <a href="{{url('/servicepro/view-space')}}/{{base64_encode($arr->space_id)}}" class="btn btn-secondary" style="margin-right: 3px;"><i class="bx bxs-show"></i></a>

                  <a href="{{url('/servicepro/edit-space')}}/{{base64_encode($arr->space_id)}}" class="btn btn-info" style="margin-right: 3px;"><i class="bx bxs-edit"></i></a>

                  <a href="javascript:void(0)" onclick="deleteConfirmation('<?php echo $arr->space_id; ?>');" class="btn btn-danger" style="margin-right: 3px;"><i class="bx bxs-trash" alt="user" title="user"></i></a>
