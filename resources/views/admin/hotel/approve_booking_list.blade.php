  ...
  function rejectBookingReqConfirmation(id) {
    toastDelete.fire({}).then(function(e) {
      ...
      if (e.value === true) {
        ...
        $.ajax({
          type: 'POST',
          url: "{{url('/servicepro/cancelRoomBookingRequestStatus')}}",
          data: {
            id: id,
            _token: CSRF_TOKEN
          },
  ...
  function deleteInvoiceConfirmation(id) {
    toastDelete.fire({}).then(function(e) {
      if (e.value === true) {
        ...
        $.ajax({
          type: 'POST',
          url: "{{url('/servicepro/deleteRoomBookingRequest')}}",
          data: {
            id: id,
            _token: CSRF_TOKEN
          },
  ...
  $(document).ready(function(){
    $('#example').on('click','.viewdetails',function(){
        ...
        if(requestId > 0){
            ...
            // AJAX request
            var url = "{{ route('getRoomInvoiceDetails',[':requestId']) }}";
...
<script>
  $('.issue_invoice').click(function() {
    ...
    // AJAX request
    $.ajax({
      url: "{{url('/servicepro/sendRoomInvoice')}}",
      type: 'post',
      data: {
        ...
        <div class="col-12">
          ...
          <tbody>
            ...
            <tr id="row{{ $arr->id }}">
              ...
              <td>
                @if($arr->request_status == 1 and $arr->approve_status == 0 and $arr->payment_status == 0)
                  {{ 'Pending' }}
                @elseif($arr->request_status == 1 and $arr->approve_status == 1 and $arr->payment_status == 0)  
                  {{ 'Invoice Issued' }}
                @elseif($arr->request_status == 1 and $arr->approve_status == 1 and $arr->payment_status == 1)
                  {{ 'Confirmed' }}
                @elseif($arr->request_status == 0 and $arr->approve_status == 0 and $arr->payment_status == 0)
                  {{ 'Cancelled by Admin' }}
                @endif
              </td>
              <!-- <td>@if($arr->booking_option == 2)
                <div class="btn-group btn-group-sm"> -->
                  ...
                  <button class="btn btn-secondary viewdetails" style="margin-right: 3px;" data-id='{{ $arr->id }}' disabled>Issue Invoice</button>
                  @else
                ...
                @if($arr->payment_status == 0) -->
                  <!-- <div class="btn-group btn-group-sm">
                    <a href="javascript:void(0)" class="btn btn-danger" style="margin-right: 3px;" disabled="true" onclick="return false">Reject Booking Request</a> -->
                    <!-- <a href="javascript:void(0)" onclick="rejectBookingReqConfirmation('<?php echo $arr->id; ?>');" class="btn btn-danger" style="margin-right: 3px;" disabled>Reject Booking Request</a> -->
                  <!-- </div> -->
                  <!-- <div class="btn-group btn-group-sm">
                    <a href="javascript:void(0)" class="btn btn-danger" style="margin-right: 3px;" disabled="true" onclick="return false">Delete Booking</a> -->
                    <!-- <a href="javascript:void(0)" onclick="deleteInvoiceConfirmation('<?php echo $arr->id; ?>');" class="btn btn-danger" style="margin-right: 3px;" disabled>Delete Booking</a> -->
                  <!-- </div> -->
                <!-- @endif
                @else  -->
                <!-- {{ "Instant Booking"}} -->
