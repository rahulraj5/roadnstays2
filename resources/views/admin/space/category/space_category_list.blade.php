...
<script type="text/javascript">
 function delete_user(amenity_id){
    $.ajaxSetup({
      headers: {
    ...
    $.ajax({
     type: 'POST',
     url: "<?php echo url('/admin/deleteHotelAmenity'); ?>",
     enctype: 'multipart/form-data',
     data:{amenity_id:amenity_id,'_token':'<?php echo csrf_token(); ?>'},
     beforeSend:function(){
  ...
  function deleteConfirmation(id) {
    toastDelete.fire({
      ...
      if (e.value === true) {
        ...
        $.ajax({
          type: 'POST',
          url: "{{url('/admin/deleteSpaceCategory')}}",
          data: {
            cat_id: id,
            _token: CSRF_TOKEN
          },
...
<script>
  $('.toggle-class').on('change', function() {
    ...
    $.ajax({
      type: "GET",
      dataType: "json",
      url: "<?php echo url('/admin/changeSpaceCategoryStatus'); ?>",
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>Space Category List</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}">Home</a></li>
                            <li class="breadcrumb-item active">Space Category List</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="row">
                            ...
                            <div class="col-md-1"><a href="{{ url('/admin/add-space-category') }}"
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @if (!$space_category_list->isEmpty())
                                            <?php $i = 1; ?>
                                            @foreach ($space_category_list as $arr)
                                                <tr id="row{{ $arr->scat_id }}">
                                                    <td>{{ $i }}</td>
                                                    <td>{{ $arr->category_name }}</td>
                                                    <!-- <td>{{ $arr->details ?? '-' }}</td> -->
                                                    <!-- <td><a href="{{url('/admin/viewSpaceCatList')}}/{{$arr->scat_id}}" class="btn btn-default btn-sm"><i class="fas fa-list"></i>  View</a></td> -->
                                                    <td class="project-state">
                                                        <input  type="checkbox" class="toggle-class" data-id="{{ $arr->scat_id }}" data-toggle="toggle" data-style="slow" data-onstyle="success" data-size="small" data-on="Active" data-off="InActive" {{ $arr->status ? 'checked' : '' }}>
                                                    </td>
                                                    <td class="text-right py-0 align-middle">
                                                        <div class="btn-group btn-group-sm">
                                                            <!-- <a href="#" class="btn btn-secondary" style="margin-right: 3px;"><i class="fas fa-eye"></i></a> -->
                                                            <a href="{{url('/admin/edit-space-category')}}/{{base64_encode($arr->scat_id)}}" class="btn btn-info" style="margin-right: 3px;"><i class="fas fa-pencil-alt"></i></a>
                                                            <a href="javascript:void(0)" onclick="deleteConfirmation('<?php echo $arr->scat_id; ?>');" class="btn btn-danger" style="margin-right: 3px;"><i class="fas fa-trash"  alt="user" title="user"></i></a>
                                                        </div>
                                                    </td>
                                                </tr>
                                                <?php $i++; ?>
